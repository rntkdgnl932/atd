...
from auction_arthdal import auction_start
...
class MyApp(QDialog):
    ...
    def initUI(self):
        ...
        # 풀버젼
        # pyinstaller --hidden-import PyQt5 --hidden-import pyserial --hidden-import requests --hidden-import chardet --add-data="C:\\my_games\\arthdal\\data_arthdal;./data_arthdal" --add-data="C:\\my_games\\arthdal\\mysettings;./mysettings" --name arthdal -i="atomic.ico" --add-data="atomic.ico;./" --icon="atomic.ico" --paths "C:\Users\1_S_3\AppData\Local\Programs\Python\Python311\Lib\site-packages\cv2" main.py
        # 업데이트버젼
        ...
        if line_[1] == "super_coob":
            ...
            self.setGeometry(20 + x_reg, 100, 950, 800)
        self.show()
    def my_title(self):
...
class FirstTab(QWidget):
    ...
    def initUI(self):
        ...
        dun_g1_stair_list = ['지역', '1', '2', '3', '4', '5', '6', '7', '8', '9']
    ...
    def onActivated_dunjeon_2_add(self):
        char_ = onCharacter
        dun_ = "시간던전_" + str(onDunjeon_2) + "_" + str(onDunjeon_2_level)
        if onCharacter == 0:
            pyautogui.alert(button='넵', text='캐릭터를 선택해 주시지예', title='뭐합니꺼')
        elif onCla == 'none':
            pyautogui.alert(button='넵', text='몇 클라인지 선택해 주시지예', title='뭐합니꺼')
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        if game_title == True:
                            ...
                            if look_title == True:
                                ...
                                # 게임 시작 화면인지 분석부터 하기
                                game_ready(v_.now_cla, character_id)
