...
sys.path.append('C:/my_games/arthdal/data_arthdal/mymodule')
...
def my_property_upload(cla):
    ...
    from auction_arthdal import mine_check
    ...
    try:
        ...
        while isstart1 is False:
            if os.path.isfile(file_path) == True:
                with open(file_path3, "r", encoding='utf-8-sig') as file:
                    ...
                    print('아스달 game server', game_server)
                    # line3 => 게임서버
            else:
        ...
        if int(result_mine[1]) != 0:
            ...
            # 6. 로컬 파일 경로 (절대 경로 사용)
            local_file_path = 'C:/my_games/arthdal/mysettings/my_property/my_property.txt'

            dir_path = "C:\\my_games\\arthdal\\mysettings\\my_property"
            ...
            # 7. 원격 파일 경로 (FTP 서버 내)
            remote_directory = '/arthdal/' + str(line_[0])  # 원격 디렉토리 경로
            remote_file_path = '/arthdal/' + str(line_[0]) + '/' + str(line_[1]) + '.txt'
